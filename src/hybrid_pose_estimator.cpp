...
HybridEstimatePoseScaleOffset(const std::vector<Eigen::Vector2d> &x0, const std::vector<Eigen::Vector2d> &x1,
    ...
    HybridPoseEstimator3 solver(x0, x1, depth0, depth1, min_depth, K0, K1, sampson_squared_weight, ransac_options.squared_inlier_thresholds_, est_config);
...
HybridEstimatePoseAndScale(const std::vector<Eigen::Vector2d> &x0, const std::vector<Eigen::Vector2d> &x1,
    ...
    // ransac_options.data_type_weights_[1] *= 0.5;
    // HybridPoseEstimator solver(x0, x1, depth0, depth1, K0, K1, sampson_squared_weight, ransac_options.squared_inlier_thresholds_, est_config);
    ...
    ransac_options.data_type_weights_[1] = ransac_options.data_type_weights_[0];
    ransac_options.squared_inlier_thresholds_[1] = ransac_options.squared_inlier_thresholds_[0];
    HybridPoseEstimatorScaleOnly3 solver(x0, x1, depth0, depth1, K0, K1, sampson_squared_weight, ransac_options.squared_inlier_thresholds_, est_config);

    PoseAndScale best_solution;
    ransac_lib::HybridRansacStatistics ransac_stats;

    HybridUncertaintyLOMSAC<PoseAndScale, std::vector<PoseAndScale>, HybridPoseEstimatorScaleOnly3> lomsac;
    lomsac.EstimateModel(ransac_options, solver, &best_solution, &ransac_stats);
    ...
    if (solver_idx == 0) {
        ...
        for (auto &pose : poses) {
            ...
            if (!est_config_.use_shift) {
                ...
                double s0 = d0_(sample[1]).dot(p3d.row(2)) / d0_(sample[1]).squaredNorm();

                p3d = p3d / s0;
                pose.t = pose.t / s0;
                p3d = (pose.R() * p3d).colwise() + pose.t;

                double scale = d1_(sample[1]).dot(p3d.row(2)) / d1_(sample[1]).squaredNorm();
...
double HybridPoseEstimator3::EvaluateModelOnPoint(const PoseScaleOffset& model, int t, int i, bool is_for_inlier) const {
    if (!is_for_inlier && est_config_.score_type == EstimatorOption::EPI_ONLY && t != 2) {
        return std::numeric_limits<double>::max();
    }
    if (!is_for_inlier && est_config_.score_type == EstimatorOption::MD_ONLY && t == 2) {
...
// Linear least squares solver. 
void HybridPoseEstimator3::LeastSquares(const std::vector<std::vector<int>>& sample, const int solver_idx, PoseScaleOffset* model) const {
    OptimizerConfig config;
    config.use_geometric = false;
...
int HybridPoseEstimatorScaleOnly3::NonMinimalSolver(const std::vector<std::vector<int>>& sample, const int solver_idx, PoseAndScale* solution) const {
    if (sample[0].size() < 3 || sample[1].size() < 5) {
        return 0;
    }
...
int HybridPoseEstimatorScaleOnly3::MinimalSolver(const std::vector<std::vector<int>>& sample,
...
double HybridPoseEstimatorScaleOnly3::EvaluateModelOnPoint(const PoseAndScale& model, int t, int i, bool is_for_inlier) const {
    if (!is_for_inlier && est_config_.score_type == EstimatorOption::EPI_ONLY && t != 2) {
        return std::numeric_limits<double>::max();
    }
    if (!is_for_inlier && est_config_.score_type == EstimatorOption::MD_ONLY && t == 2) {
...
// Linear least squares solver. 
void HybridPoseEstimatorScaleOnly3::LeastSquares(const std::vector<std::vector<int>>& sample, const int solver_idx, PoseAndScale* model) const {
    OptimizerConfig config;
    config.use_geometric = false;
